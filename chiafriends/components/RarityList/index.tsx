...
const RarityList = ({
	...
	const changeSort = (sortBy: string) => {
		router.push(`/chiafriends/?sortBy=${sortBy}`, `/chiafriends/?sortBy=${sortBy}`);
	};

	const openToken = () => {
		router.push(`/chiafriends/?viewId=${tokenId}`, `/chiafriends/view/${tokenId}`);
	};
					...
					<div className="flex justify-between items-end pb-4 mb-6 border-b border-gray-200 dark:border-gray-700 max-w-screen-xl mx-auto">
						<div>
							<h3 className="font-semibold ml-2">
								{meta.totalCount} Total CHIA FRIENDS<SocialLinks />
							</h3>
						</div>
