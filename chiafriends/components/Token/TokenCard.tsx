...
const TokenCard = ({ token }: any) => {
  return (
    <Link
      href={`/chiafriends/?viewId=${token.id}`}
      as={`/chiafriends/view/${token.id}`}
      passHref
      scroll={false}
    >
      <a>
                  ...
                  <div>
                    ...
                    <div className="bg-black block rounded-md overflow-hidden transition duration-150 ease hover:scale-105">
                      <img
                        ...
                        src={`https://bafybeigzcazxeu7epmm4vtkuadrvysv74lbzzbl2evphtae6k57yhgynp4.ipfs.nftstorage.link/${token.id}.png`}
                        alt={token.name}
                      />
                    </div>
